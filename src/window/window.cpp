...
void Window::CompileShaders()
{
    ...
    unsigned char *data = stbi_load("D:\\DEV\\MY_GITHUB\\Midnight-Engine\\build\\Debug\\images\\textures\\icon.jpg", &width, &height, &nrChannels, 0);
    if (data) 
    {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // load image, create texture and generate mipmaps
    data = stbi_load("D:\\DEV\\MY_GITHUB\\Midnight-Engine\\build\\Debug\\images\\textures\\icon.jpg", &width, &height, &nrChannels, 0);
    if (data)
    {
